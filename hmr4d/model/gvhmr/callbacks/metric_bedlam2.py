...
class MetricMocap(pl.Callback):
    def __init__(self):
        ...
        self.target_dataset_id = "bedlam2_test"
    ...
    def on_predict_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=0):
        ...
        target_w_output = self.smplx(**target_w_params)
        ...
        pred_w_verts = smpl_out.vertices
        pred_w_j3d = smpl_out.joints

        del smpl_out  # Prevent OOM

        batch_eval = {
            "pred_j3d_glob": pred_w_j3d,
            "target_j3d_glob": target_w_j3d,
            "pred_verts_glob": pred_w_verts,
            "target_verts_glob": target_w_verts,
        }
        #TODO: consider shoe height for foot sliding
        try:
            global_metrics = compute_global_metrics(batch_eval, mask=mask, chunk_length=-1)
            for k in global_metrics:
                self.metric_aggregator[k][vid] = as_np_array(global_metrics[k])
        except Exception as e:
            self.failed_vids.append(vid)
            Log.error(f"Error in compute_global_metrics: {vid}, total failed: {len(self.failed_vids)}")
        ...
        if False:  # Render incam
            ...
            out_fn = f"outputs/dump_render_b2test{split}/{vname}.mp4"
    ...
    def on_predict_epoch_end(self, trainer, pl_module):
        ...
        if "wa2_mpjpe" in self.metric_aggregator:
            monitor_metric = "wa2_mpjpe"
        else:
        ...
        if len(mm_per_seq) > 0:
            sorted_mm_per_seq = sorted(mm_per_seq.items(), key=lambda x: x[1], reverse=False)
