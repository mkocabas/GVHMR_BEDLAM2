...
def compute_global_metrics(batch, mask=None, chunk_length=100):
    ...
    chunk_length = chunk_length if chunk_length > 0 else seq_length
    wa2_mpjpe, waa_mpjpe = [], []
...
def compute_foot_sliding(target_verts, pred_verts, thr=1e-2):
    ...
    if target_verts.shape[-2] == 6890:
        ...
        # Foot vertices idxs for SMPL
        foot_idxs = [3216, 3387, 6617, 6787]
    elif target_verts.shape[-2] == 10475:
        # Foot vertices idxs for SMPLX
        foot_idxs = [8549, 8702, 8915, 6855]
    else:
        raise ValueError(f"Unknown number of vertices: {target_verts.shape[-2]}")
