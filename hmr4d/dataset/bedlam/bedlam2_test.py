...
class Bedlam2TestDataset(ImgfeatMotionDatasetBase):
    ...
    def _load_dataset(self):
        Log.info(f"[BEDLAM-2 TEST] Loading from {self.root}")
        tic = time()
        smplpose_file = f'smplpose_test_v3_w_droid.pt'

        if os.path.exists(self.root / smplpose_file):
            print(f'Loading {smplpose_file} from {self.root}')
            self.motion_files = torch.load(self.root / smplpose_file)
        else:
            self.smplx = make_smplx('smplx-bedlam2', gender='neutral').to(device)

            cam_root_dir = '/home/muhammed/projects/mfv_dev/results/20250519_b2_testset'
            # motion_files = glob.glob(f'{self.root}/training_labels_30fps/*/*.pt')
            import pandas as pd
            shoe_offsets = pd.read_csv('/home/data/datasets/BEDLAM2_0/images/source_data/shoe_offsets.csv')
            shoe_offsets = dict(zip(shoe_offsets['shoe_id'], shoe_offsets['offset']))
            shoe_offsets['n/a'] = 0.0
            
            # img_feat_files = sorted(glob.glob(f'{self.root}/imgfeats/prhmr_{self.version}/*/*.pt'))
            gt_labels = sorted(glob.glob(f'{self.root}/test_labels_30fps/*/*.pt'))
            img_feat_files = []
            for gt_label in gt_labels:
                imf = gt_label.replace('test_labels_30fps', 'imgfeats/hmr2')
                if os.path.exists(imf):
                    img_feat_files.append(imf)
                else:
                    print(f'{imf} does not exist')
            ...
            for imf in pbar:
                ...
                mf = imf.replace(f'imgfeats/{self.version}', 'test_labels_30fps')
                
                cam_file = f'{cam_root_dir}/{imf.split("/")[-2]}/mp4/{imf.split("/")[-1].split("-")[0]}/results.pkl'
                try:
                    droid_cam_ext = joblib.load(cam_file)['camera']['pred_cam_ext']
                except Exception as e:
                    print(e)
                    print(f'{cam_file} is problematic')
                    print(f'Exiting...')
                    exit()
                
                assert os.path.exists(mf), f'{mf} does not exist'
                shoe_id = open(mf.replace('.pt', '_shoe.txt')).read().strip()
                shoe_offset_ = shoe_offsets[shoe_id]
                motion_dict = torch.load(mf)
                
                frame_ids = [int(x.split('/')[-1].split('_')[-1].split('.')[0]) for x in motion_dict['imgnames']]
                
                if frame_ids[0] > 0:
                    print(f'{imf} starts at {frame_ids[0]}')
                    
                droid_cam_ext = droid_cam_ext[frame_ids]
                ...
                j3d = self.smplx(betas=torch.from_numpy(motion_dict['betas'][0:1]).to(device)).joints[0]

                # assert len(droid_cam_ext) == len(motion_dict['valid_mask'])
                n_motion_dict = {
                    ...
                    'droid_cam_ext': droid_cam_ext,
                    'skeleton': j3d.cpu(),
                    'valid_start_end': (start, end),
                    'shoe_offset': shoe_offset_,
                }
        ...
        Log.info(f"[BEDLAM-2 TEST] Motion files loaded. Elapsed: {time() - tic:.2f}s")
    ...
    def _load_data(self, idx):
        ...
        mfname = self.root / mid.replace('imgfeats/prhmr_b2nips', f'imgfeats/{self.version}')
        f_imgseq = torch.load(mfname)
        ...
        length = data['poses_world'].shape[0]
        start = 0
        end = length
    ...
    def _process_data(self, data, idx):
        ...
        pred_cam_ext = data["droid_cam_ext"]
        
        # R_c2gv = get_R_c2gv(T_w2c[:, :3, :3], gravity_vec)  # (F, 3, 3)
        R_c2gv = get_R_c2gv(pred_cam_ext[:, :3, :3], gravity_vec)  # (F, 3, 3)
        ...
        cam_angvel = compute_cam_angvel(pred_cam_ext[:, :3, :3])  # (F, 6)
        
        # Returns: do not forget to make it batchable! (last lines)
        max_len = length

        mid = Path(self.idx2meta[idx])
        # video_path = self.root / "videos_test" / mid.parts[-2] / "mp4" / (mid.stem.split('-')[0] + '.mp4')
        video_path = Path(mid.parts[-2]) / "mp4" / (mid.stem.split('-')[0] + '.mp4')
        
        return_data = {
            "meta": {"data_name": "bedlam2_test", "idx": idx, "vid": video_path, "dataset_id": "bedlam2_test"},
            "length": length,
        ...
        if False:  # cam-overlay
            ...
            # from hmr4d.utils.video_io_utils import pad_image
            # images_proc = []
            # for img in images:
            #     images_proc.append(pad_image(img))
            # images = np.stack(images_proc, axis=0)
            ...
            save_video(img_overlay, f".tmp/bedlam2_test_data_{idx:02d}.mp4", crf=23)
